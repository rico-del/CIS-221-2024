...
//let current Temperature = Math.round(currentweather.data.temperature.current);
//document.write(currentTemperature);
//display the temperature

let currentTemperatureValue = document.querySelector("#currenttemperaturevalue");

//currente TemperatureValue.innerHTML = currentTemperature;
...
let currentWeatherlcon = document.querySelector("#weather-app-icon");
currentWeathericon.innerHTML = <img src="$(currentweather.data.condition.ican_url)}" alt= "not loading" class="weather-app-icon" />;
console.log(currentweather.data);

//display the humidity let humidity = document.querySelector("#Humidity"); humidity.innerHTML = '$(currentweather.data.temperature.humidity}%';
//display the wind speed let windSpeed = document.querySelector("#windspeed"); windSpeed.innerHTML = '${currentweather.data.wind.speed) knots';
//display day and time let currentTime = document.querySelector("#currenttime"); let date = new Date(currentweather.data.time * 1000); currentTime.innerHTML = formatDate(date);
}
